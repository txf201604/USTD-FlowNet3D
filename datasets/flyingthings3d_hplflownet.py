...
class FT3D(SceneFlowDataset):
    def __init__(self, root_dir, nb_points, mode, voxel_size=0.05):
        ...
        Args:
            root_dir: str. Path to root directory containing the datasets.
            nb_points: int. Maximum number of points in point clouds.
            mode: str, ['train' | 'val' | 'test']. Dataset mode.
    ...
    def load_sequence(self, idx):
        ...
        Returns:
            sequence : list(np.array, np.array)
                List [pc1, pc2] of point clouds between which to estimate scene flow.
                pc1 has size n x 3 and pc2 has size m x 3.

            ground_truth : list(np.array, np.array)
                List [mask, flow].
                mask has size n x 1. mask is binary with zeros indicating where the flow is not valid/occluded.
                flow is the ground truth scene flow between pc1 and pc2.
        """
        ...
        return sequence, ground_truth, self.filenames[idx].split('/')[-1]
