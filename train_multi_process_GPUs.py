...
def train_ddp_worker(rank, world_size, dataset_name, nb_iter, batch_size, max_points, nb_epochs, ckpt_path, port_num, voxel_size):
    ...
    scene_flow = FPTOT(nb_iter=nb_iter, voxel_size=voxel_size).to(rank)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--voxel_size", type=float, default=0.07, help="voxel size")

    parser.add_argument("--ckpt_path", type=str, default=None, help="check point path to load")

    parser.add_argument("--port_num", type=str, default='12355', help="localhost port number to init process group")
    ...
    mp.spawn(train_ddp_worker, nprocs=n_gpus, args=(n_gpus, args.dataset, args.nb_iter, args.batch_size, args.nb_points,
                                                    args.nb_epochs, args.ckpt_path, args.port_num, args.voxel_size))
    # train_ddp_worker(n_gpus, 1, args.dataset, args.nb_iter, args.batch_size, args.nb_points, args.nb_epochs,
    #                  args.ckpt_path, args.port_num, args.voxel_size)
