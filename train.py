...
from models.scene_flow_fpt import FPTOT
...
def main(dataset_name, nb_iter, batch_size, max_points, nb_epochs, ckpt_path, voxel_size):
    """
    ...
    trainloader = DataLoader(ft3d_train, batch_size=batch_size, pin_memory=True, num_workers=1, shuffle=True,
                             collate_fn=Batch, drop_last=True)

    # Model
    scene_flow = FPTOT(nb_iter=nb_iter, voxel_size=voxel_size).to(device)
    scene_flow = torch.nn.parallel.DataParallel(scene_flow, device_ids=device_ids)
    ...
    # Train
    train_worker(scene_flow, trainloader, 10, optimizer, scheduler, path2log, nb_epochs, start_epoch)

def train_worker(scene_flow, trainloader, delta, optimizer, scheduler, path2log, nb_epochs, start_epoch):
    """
    ...
    for epoch in range(epoch_start, nb_epochs + epoch_start):
        ...
        print("Epoch {0:d} - It. {1:d}: loss = {2:e}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="HPLFlowNet", help="Training dataset. Only support HPLFlowNet")
    parser.add_argument("--batch_size", type=int, default=2, help="Batch size.")
    parser.add_argument("--nb_epochs", type=int, default=60, help="Number of epochs.")
    parser.add_argument("--nb_points", type=int, default=8192, help="Maximum number of points in point cloud.")
    ...
    parser.add_argument("--ckpt_path", type=str, default=None, help="check point path to load")
    args = parser.parse_args()

    main(args.dataset, args.nb_iter, args.batch_size, args.nb_points, args.nb_epochs, args.ckpt_path, args.voxel_size)
